...
function StatCard({
  ...
  return (
    <motion.div
      ...
      className="glass rounded-xl p-4 glass-hover flex flex-col"
    >
...
function HoldingRow({
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-3 py-3 px-3 rounded-lg hover:bg-white/[0.03] transition-colors border-b border-white/[0.04] last:border-b-0"
    >
      {/* Stock name + ticker */}
      <div className="flex-1 min-w-0">
        <div className="text-[12px] font-medium text-white truncate">{holding.id}</div>
        <div className="text-[9px] text-zinc-600 font-mono">{holding.id}</div>
      </div>

      {/* Shares */}
      <div className="w-[80px] text-right">
        <div className="text-[11px] font-mono text-zinc-300">{holding.shares.toLocaleString()}</div>
        <div className="text-[8px] text-zinc-600">股數</div>
      </div>

      {/* Avg Cost */}
      <div className="w-[90px] text-right">
        <div className="text-[11px] font-mono text-zinc-300">{fmt(holding.avg_cost)}</div>
        <div className="text-[8px] text-zinc-600">均價</div>
      </div>

      {/* Current Price */}
      <div className="w-[90px] text-right">
        <div className={`text-[11px] font-mono font-medium ${isUp ? 'text-[#00FF88]' : 'text-[#FF4D67]'}`}>
          {fmt(holding.current)}
        </div>
        <div className="text-[8px] text-zinc-600">現價</div>
      </div>

      {/* P&L */}
      <div className="w-[100px] text-right">
        <div className={`text-[11px] font-mono font-bold ${isUp ? 'text-[#00FF88]' : 'text-[#FF4D67]'}`}>
          {isUp ? '+' : ''}{fmt(pl)}
        </div>
        <div className={`text-[9px] font-mono ${isUp ? 'text-[#00FF88]/70' : 'text-[#FF4D67]/70'}`}>
          {fmtPct(returnPct)}
        </div>
      </div>
    </motion.div>
...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="glass-dark rounded-lg px-3 py-2 text-[11px] shadow-lg border border-white/10">
  ...
  return (
    <section className="py-8">
      <div className="max-w-[1440px] mx-auto px-4 lg:px-8">
        ...
        >
          <div>
            <h2 className="text-lg font-bold text-white">個人投資儀表板</h2>
            <p className="text-[11px] text-zinc-500 mt-0.5">Portfolio Dashboard</p>
          </div>
        ...
        {!hasPortfolio ? (
          ...
          >
            ...
            <p className="text-[10px] text-zinc-700">請先連接您的帳戶或上傳持股資訊</p>
          </motion.div>
        ) : (
          <>
            {/* ── 1. Main Stats Row ── */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2">
                ...
                >
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-[11px] font-medium text-zinc-400">持股明細</span>
                    <span className="text-[9px] text-zinc-600 ml-auto">{holdings.length} 檔</span>
                  </div>

                  {/* Table header */}
                  <div className="flex items-center gap-3 px-3 pb-2 border-b border-white/[0.06] text-[9px] text-zinc-600 font-medium uppercase tracking-wider">
                    <div className="flex-1">標的</div>
                    <div className="w-[80px] text-right">股數</div>
                    <div className="w-[90px] text-right">均價</div>
                    <div className="w-[90px] text-right">現價</div>
                    <div className="w-[100px] text-right">損益</div>
                  </div>
              ...
              >
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-[11px] font-medium text-zinc-400">資產配置</span>
                </div>
                ...
                {pieData.length > 0 ? (
                  ...
                  <div className="flex items-center justify-center h-[260px] text-[11px] text-zinc-600">
                    無資產資料
                  </div>
                )}
